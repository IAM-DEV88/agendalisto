...
-- NOTA: usa prefijo agendaya_ para coexistir con encuentrosvip en el mismo proyecto Supabase
...
CREATE TABLE IF NOT EXISTS public.agendaya_business_categories (
...
-- Tabla de negocios
CREATE TABLE IF NOT EXISTS public.agendaya_businesses (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE conname = 'fk_business_id' AND conrelid = 'public.agendaya_profiles'::regclass
  ) THEN
    -- Si no existe, añadirla
    ALTER TABLE public.agendaya_profiles 
      ADD CONSTRAINT fk_business_id 
      FOREIGN KEY (business_id) 
      REFERENCES public.agendaya_businesses(id) 
      ON DELETE SET NULL;
  END IF;
END;
$$;
...
CREATE TABLE IF NOT EXISTS public.agendaya_services (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE TABLE IF NOT EXISTS public.agendaya_business_hours (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  business_id UUID REFERENCES public.agendaya_businesses(id) ON DELETE CASCADE NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.agendaya_business_config (
  business_id UUID PRIMARY KEY REFERENCES public.agendaya_businesses(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.agendaya_appointments (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  business_id UUID REFERENCES public.agendaya_businesses(id) ON DELETE CASCADE NOT NULL,
...
-- Tabla de reseñas
CREATE TABLE IF NOT EXISTS public.agendaya_reviews (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  appointment_id UUID REFERENCES public.agendaya_appointments(id) ON DELETE CASCADE NOT NULL,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  business_id UUID REFERENCES public.agendaya_businesses(id) ON DELETE CASCADE NOT NULL,
...
-- NOTA: El trigger handle_new_user() está definido en el proyecto encuentrosvip
-- (supabase/coexistence.sql). Crea perfiles para AMBAS apps simultáneamente.
-- NO redefinir aquí sin mantener compatibilidad con encuentrosvip.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_agendaya_profiles_updated_at'
  ) THEN
    CREATE TRIGGER update_agendaya_profiles_updated_at 
      BEFORE UPDATE ON public.agendaya_profiles
      FOR EACH ROW 
  ...
  IF NOT EXISTS (
    ...
    CREATE TRIGGER update_businesses_updated_at 
      BEFORE UPDATE ON public.agendaya_businesses
      FOR EACH ROW 
      EXECUTE FUNCTION public.update_modified_column();
  END IF;
  ...
  IF NOT EXISTS (
    ...
    CREATE TRIGGER update_services_updated_at 
      BEFORE UPDATE ON public.agendaya_services
      FOR EACH ROW 
      EXECUTE FUNCTION public.update_modified_column();
  END IF;
  ...
  IF NOT EXISTS (
    ...
    CREATE TRIGGER update_appointments_updated_at 
      BEFORE UPDATE ON public.agendaya_appointments
      FOR EACH ROW 
      EXECUTE FUNCTION public.update_modified_column();
  END IF;
END;
...
-- ===== POLÍTICAS PARA AGENDAYA PROFILES =====
...
DROP POLICY IF EXISTS "agendaya_users can update their own profile" ON public.agendaya_profiles;
CREATE POLICY "agendaya_users can update their own profile" 
  ON public.agendaya_profiles 
  FOR UPDATE 
  USING (auth.uid() = id);
...
DROP POLICY IF EXISTS "Businesses are viewable by everyone" ON public.agendaya_businesses;
CREATE POLICY "Businesses are viewable by everyone" 
  ON public.agendaya_businesses 
  FOR SELECT 
  USING (true);

-- Política para crear negocios (usuario autenticado)
DROP POLICY IF EXISTS "Users can create businesses" ON public.agendaya_businesses;
...
DROP POLICY IF EXISTS "Owners can update their businesses" ON public.agendaya_businesses;
...
DROP POLICY IF EXISTS "Owners can delete their businesses" ON public.agendaya_businesses;
...
DROP POLICY IF EXISTS "Services are viewable by everyone" ON public.agendaya_services;
CREATE POLICY "Services are viewable by everyone" 
  ON public.agendaya_services 
  FOR SELECT 
  USING (true);

-- Política para crear servicios (solo dueño del negocio)
DROP POLICY IF EXISTS "Business owners can insert services" ON public.agendaya_services;
...
DROP POLICY IF EXISTS "Business owners can update services" ON public.agendaya_services;
...
DROP POLICY IF EXISTS "Business owners can delete services" ON public.agendaya_services;
...
DROP POLICY IF EXISTS "Business hours are viewable by everyone" ON public.agendaya_business_hours;
...
DROP POLICY IF EXISTS "Business owners can insert business hours" ON public.agendaya_business_hours;
...
DROP POLICY IF EXISTS "Business owners can update business hours" ON public.agendaya_business_hours;
...
DROP POLICY IF EXISTS "Business owners can delete business hours" ON public.agendaya_business_hours;
...
CREATE POLICY "Users can view their own appointments" 
  ON public.agendaya_appointments 
  FOR SELECT 
...
DROP POLICY IF EXISTS "Reviews are viewable by everyone" ON public.agendaya_reviews;
...
DROP POLICY IF EXISTS "Users can create reviews for completed appointments" ON public.agendaya_reviews;
...
DROP POLICY IF EXISTS "Users can update their own reviews" ON public.agendaya_reviews;
...
DROP POLICY IF EXISTS "Users can delete their own reviews" ON public.agendaya_reviews;
...
DROP POLICY IF EXISTS "Categories are viewable by everyone" ON public.agendaya_business_categories;
CREATE POLICY "Categories are viewable by everyone" 
  ON public.agendaya_business_categories 
  FOR SELECT 
  USING (true);
...
DROP POLICY IF EXISTS "Business config are viewable by everyone" ON public.agendaya_business_config;
CREATE POLICY "Business config are viewable by everyone"
  ON public.agendaya_business_config FOR SELECT
  USING (true);

-- Política para crear configuración (solo dueño del negocio)
DROP POLICY IF EXISTS "Business owners can insert business config" ON public.agendaya_business_config;
CREATE POLICY "Business owners can insert business config"
  ON public.agendaya_business_config FOR INSERT
  WITH CHECK (
    auth.uid() IN (
      SELECT owner_id FROM public.agendaya_businesses WHERE id = business_id
    )
...
DROP POLICY IF EXISTS "Business owners can update business config" ON public.agendaya_business_config;
CREATE POLICY "Business owners can update business config"
  ON public.agendaya_business_config FOR UPDATE
  USING (
    auth.uid() IN (
      SELECT owner_id FROM public.agendaya_businesses WHERE id = business_id
    )
...
BEGIN
  IF NOT EXISTS (
    ...
    CREATE TRIGGER update_business_config_updated_at
      BEFORE UPDATE ON public.agendaya_business_config
      FOR EACH ROW
      EXECUTE FUNCTION public.update_modified_column();
  END IF;
END;

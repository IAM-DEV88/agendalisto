          ...
          <div className="hidden sm:ml-6 sm:flex sm:items-center sm:space-x-4">
            ...
            {!isLoggedIn && (
              <div className="flex items-center space-x-2 ml-4">
                ...
                <Link to="/register" className="px-5 py-2 text-sm font-bold text-white bg-primary-600 hover:bg-primary-500 rounded-lg shadow-md shadow-primary-500/20 transition-all hover:-translate-y-0.5">
                  🌐 Mi web gratis
                </Link>
              </div>
            )}
            ...
            <div className="pt-4 mt-4 border-t border-slate-100 dark:border-slate-800">
              {!isLoggedIn ? (
                <div className="grid grid-cols-2 gap-4">
                  <Link to="/login" onClick={closeMenus} className="flex items-center justify-center px-4 py-3 text-base font-bold text-slate-700 dark:text-slate-300 rounded-lg bg-slate-100 dark:bg-slate-800">
                    Entrar
                  </Link>
                  <Link to="/register" onClick={closeMenus} className="flex items-center justify-center px-4 py-3 text-base font-bold text-white rounded-lg bg-primary-600">
                    🌐 Mi web gratis
                  </Link>
                </div>
              ) : (
                <div className="space-y-2">
                  <Link to="/dashboard" onClick={closeMenus} className="flex items-center px-4 py-3 text-base font-bold text-slate-700 dark:text-slate-300 rounded-lg hover:bg-primary-50 dark:hover:bg-slate-800">
                  ...
                  {staffRole && (
                    <Link to="/moderator/dashboard" onClick={closeMenus} className="flex items-center px-4 py-3 text-base font-bold text-slate-700 dark:text-slate-300 rounded-lg hover:bg-primary-50 dark:hover:bg-slate-800">
                      <Shield className="mr-3 h-4 w-4" />
                      Moderación
                    </Link>
                  )}
                  {hasBusiness && (
                    <Link to="/business/dashboard" onClick={closeMenus} className="flex items-center px-4 py-3 text-base font-bold text-slate-700 dark:text-slate-300 rounded-lg hover:bg-primary-50 dark:hover:bg-slate-800">
                      <svg className="mr-3 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                      Mi Negocio
                    </Link>
                  )}
                  {staffRole === 'admin' && (
                    <Link to="/admin/dashboard" onClick={closeMenus} className="flex items-center px-4 py-3 text-base font-bold text-slate-700 dark:text-slate-300 rounded-lg hover:bg-primary-50 dark:hover:bg-slate-800">
                      <svg className="mr-3 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                      Administración
                    </Link>
                  )}
                  {canCreateMore && (
                    <Link to="/business/register" onClick={closeMenus} className="block px-4 py-3 text-base font-bold text-primary-600 dark:text-primary-400 rounded-lg hover:bg-primary-50 dark:hover:bg-slate-800">
                      + Crear negocio
                    </Link>
                  )}

...
export default function ShareButton({
  ...
  const dropdown = open && (
    ...
    >
      <div className="p-3 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
        ...
        <button type="button" onClick={() => setOpen(false)} aria-label="Cerrar menú de compartir" className="p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 transition-colors">
      ...
      <div className="p-2 space-y-1">
        {typeof navigator.share === 'function' && (
          <button onClick={handleNativeShare}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-all text-sm font-bold text-slate-700 dark:text-slate-300">
  ...
  return (
    <div className="relative inline-flex">
      <button ref={btnRef} type="button" onClick={toggle} className={btnCls} title="Compartir" aria-label="Compartir">
        {variant === 'icon' ? (
